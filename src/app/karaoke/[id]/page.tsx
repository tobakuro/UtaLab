...
export default function KaraokePage() {
  ...
  const { recordPitch, finish, reset } = useScoring(id);

  useEffect(() => {
    fetch(`/analyzed/${id}/melody.json`)
      .then((r) => r.json())
      .then(setMelody)
      .catch(() => setMelody(null));
  }, [id]);
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-6 bg-gray-950 p-8">
      <h1 className="text-2xl font-bold text-white">{melody?.title ?? '読み込み中...'}</h1>
